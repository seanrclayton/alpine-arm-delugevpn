...
echo "Application is defined as $APPLICATION"
...
if [[ ! -z "${VPN_INPUT_PORTS}" ]]; then
	...
	read -ra -d "," vpn_input_ports_array <<< "${VPN_INPUT_PORTS}"
...
if [[ ! -z "${VPN_OUTPUT_PORTS}" ]]; then
	...
	read -ra -d "," outgoing_ports_lan_array <<< "${VPN_OUTPUT_PORTS}"
fi
...
## for alpine @ clayton
#docker_ip=$(ifconfig "${docker_interface}" | grep -P -o -m 1 '(?<=addr\s)[^\s]+')
docker_ip=$(ifconfig "${docker_interface}" | grep -P -o -m 1 '(?<=addr:)[^\s]+')
...
# split comma separated string into array from LAN_NETWORK env variable
 read -d "," -ra lan_network_array <<< "${LAN_NETWORK}"

# split comma separated string into array from VPN_REMOTE_PORT env var
 read -ra -d "," vpn_remote_port_array <<< "${VPN_REMOTE_PORT}"


echo "lan_network_array is ${lan_network_array}"
# ip route
###
